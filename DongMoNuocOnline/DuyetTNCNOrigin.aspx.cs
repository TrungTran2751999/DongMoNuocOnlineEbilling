...
{
    public partial class DuyetTNCNOrigin : Authentication
    {
        ...
        private string id;
        private readonly HopDongDao _hopDongDao = new HopDongDao();
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                containerTNCN.Attributes["style"] = "display:block";
                containerTBQHTN.Attributes["style"] = "display:none";
            }
        ...
        private void LoadReferences()
        {
            ...
            divCR.Visible = false;
            ddlTT.Attributes["onchange"] = "onchangeTrangThai()";
        }
        ...
        protected void btnBaoCao_Click(object sender, EventArgs e)
        {
            ...
            else if (TTSlected == "TNCN")
            {
                divDuyetTNCN.Visible = true;
                divCR.Visible = false;
                BindDataForDuyetTNCN("Admin");
            }
            else
            {
                divDuyetTNCN.Visible = false;
        ...
        private void BindDataForDuyetTNCN(string type)
        {
            try
            {
                //var list = new ReportClass().KH_LocKHChuaTT(thang, nam, tenDuong);
                var list = _hopDongDao.GetList(type);
                gvList.DataSource = list;
                gvList.PagerInforText = list.Count.ToString();
                gvList.DataBind();
                upnlDuyetTNCN.Update();
            }
            catch (Exception ex)
            {
                DoError(new Message(MessageConstants.E_EXCEPTION, MessageType.Error, ex.Message, ex.StackTrace));
